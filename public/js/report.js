$(document).ready(function() {
    ...
    $('#expense_category_report_table').DataTable({
        ...
        serverSide: true,
        ajax: {
            url: '/reports/expense-category-wise',
            data: function(d) {
        ...
        fnDrawCallback: function(oSettings) {
            ...
            __currency_convert_recursively($('#expense_category_report_table'));
        },
    });
    ...
    stock_report_table = $('#stock_report_table').DataTable({
        ...
        columns: [
            ...
            { data: 'stock_purchase_price', name: 'stock_purchase_price', searchable: false },
            { data: 'stock_sell_price', name: 'stock_sell_price', searchable: false },
            { data: 'potential_price', name: 'potential_price', searchable: false },
    ...
    if ($('#register_report_date_filter').length == 1) {
        ...
        $('#register_report_date_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#register_report_date_filter').val('');
            updateRegisterReport();
        });
        $('#register_report_date_filter')
...
function updateRegisterReport() {
    var start = '';
    var end = '';
    if ($('#register_report_date_filter').val()) {
        start = $('input#register_report_date_filter')
            .data('daterangepicker')
            .startDate.format('YYYY-MM-DD');
        end = $('input#register_report_date_filter')
            .data('daterangepicker')
            .endDate.format('YYYY-MM-DD');
    }
...
function updateProfitLoss() {
    ...
    var loader = __fa_awesome();
    $(
        '.opening_stock, .total_income, .total_transfer_shipping_charges, .closing_stock, .total_sell, .total_purchase, \
        .total_expense, .net_profit, .total_adjustment, .total_recovered, .total_sell_discount, \
        .total_purchase_discount, .total_purchase_return, .total_sell_return, .gross_profit','.opening_stock_by_sell','.closing_stock_by_sell'
    ).html(loader);
