        ...
        if (request()->ajax()) {
            $accounts = Account::leftjoin('account_transactions as AT', function ($join) {
                ...
                $join->whereNull('AT.deleted_at');
                                ...
                                ->select(['bank_name','name', 'account_number', 'accounts.note', 'accounts.id',
                                ...
                                if (!empty($row->sub_type)) {
                                    ...
                                    if (!empty($row->transaction->type)) {
                                        if ($row->transaction->type == 'purchase') {
                                            ...
                                            $details ='<b>Expense</b><br><b>'.
                                            __('sale.invoice_no') . ':</b> ' . $row->transaction->ref_no;
                                        }elseif ($row->transaction->type == 'income') {
                                            $details ='<b>Income</b><br>'. '<b><br><b>'.
                                            __('sale.invoice_no') . ':</b> ' . $row->transaction->ref_no;
                                        }
                                    }
                                }
    ...
    public function open($id)
    {
        ...
        if (request()->ajax()) {
            try {
                ...
                $output = ['success' => true,
                                    'msg' => __("Account Open Successfully !")
                                    ];
            } catch (\Exception $e) {
                \Log::emergency("File:" . $e->getFile(). "Line:" . $e->getLine(). "Message:" . $e->getMessage());
            ...
            return DataTables::of($accounts)
                            ...
                            ->editColumn('sub_type', function ($row) {
                                ...
                                if (!empty($row->sub_type)) {
                                    ...
                                    if (!empty($row->transaction->type)) {
                                        if ($row->transaction->type == 'purchase') {
                                            $details ='<b>Purchase</b><br>'. '<b>' . __('purchase.supplier') . ':</b> ' . $row->transaction->contact->name . '<br><b>'.
                                            __('purchase.ref_no') . ':</b> ' . $row->transaction->ref_no;
                                        } elseif ($row->transaction->type == 'sell') {
                                            $details ='<b>Sell</b><br>'. '<b>' . __('contact.customer') . ':</b> ' . $row->transaction->contact->name . '<br><b>'.
                                            __('sale.invoice_no') . ':</b> ' . $row->transaction->invoice_no;
                                        }elseif ($row->transaction->type == 'expense') {
                                            $details ='<b>Expense</b><br><b>'.
                                            __('sale.invoice_no') . ':</b> ' . $row->transaction->invoice_no;
                                        ...
                                        $details.='<br> <b>Pay reference No </b>'.$row->transaction->ref_no;
                                    }
                                }
