...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        $sells_chart_1=new salechart();
  /*       $sells_chart_1 =$sells_chart_1=>multi('line', 'highcharts')
                            ->title(' ')
                            ->template('material')
                            ->labels($labels)
   ...
   //     return view('home.index', compact('date_filters', 'sells_chart_1', 'sells_chart_2', 'widgets'));
        return view('home.index', compact('date_filters', 'widgets'));
    }
    ...
    public function getTotals()
    {
        if (request()->ajax()) {
            ...
            $income = $this->transactionUtil->getIncomeTotals($business_id, $start, $end);
            $expense = $this->transactionUtil->getExpenseTotals($business_id, $start, $end);
            ...
            $contact=Contact::all();
            $transaction=Transaction::whereIn('type',['sell','purchase'])->get();
            ...
            $output['total_expense'] = $expense['total_expense'];
            $output['expense_due'] = $expense['expense_due'];
            $output['total_income'] = $income['total_income'];
            $output['income_due'] = $income['income_due'];
            $output['total_customer'] = $contact->where('type','customer')->count();
            $output['total_supplier'] = $contact->where('type','supplier')->count();
            $output['total_sell_item'] = $transaction->where('type','sell')->count();
            $output['total_purchase_item'] = $transaction->where('type','purchase')->count();
            ...
            return Datatables::of($dues)
                ...
                ->editColumn('ref_no', function ($row) {
                    if (auth()->user()->can('purchase.view')) {
                        return  '<a href="#" data-href="' . route('purchases.show', [$row->id]) . '"
            ...
            return Datatables::of($dues)
                ...
                ->editColumn('invoice_no', function ($row) {
                    if (auth()->user()->can('sell.view')) {
                        return  '<a href="#" data-href="' . route('sells.show', [$row->id]) . '"
