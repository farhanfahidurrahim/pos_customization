...
class ProductUtil extends Util
{
    ...
    public function updateProductQuantity2($location_id, $product_id, $variation_id, $new_quantity, $old_quantity = 0)
    {
     ...
     * @return array
     */
    public function checkRecipeQty($product_id, $location_id = null)
    {
        $package =\App\Package::with('raws','raws.product')
                              ->where('product_id',$product_id)
                              ->first();
            foreach($package->raws as $row){
     ...
     */
    public function calculateInvoiceTotal($products, $tax_id=null, $discount = null)
    {
        if (empty($products)) {
            return false;
        }
