...
class TransactionUtil extends Util
{
    ...
    public function createSellTransaction($business_id, $input, $invoice_total, $user_id, $uf_data = true)
    {
        ...
        $transaction = Transaction::create([
            ...
            'ref_no' => isset($input['ref_no']) ? $input['ref_no'] : null,
            'total_before_tax' => $invoice_total['total_before_tax'],
            'transaction_date' => $input['transaction_date'],
            'tax_id' => isset ($input['tax_rate_id']) ? $input['tax_rate_id']:null,
            'discount_type' => $input['discount_type'],
            'discount_amount' => $uf_data ? $this->num_uf($input['discount_amount']) : $input['discount_amount'],
            'tax_amount' => $input['tax_calculation_amount'],
        ...
        foreach ($products as $product) {
            ...
            if (!empty($product['transaction_sell_lines_id'])) {
                ...
                $uf_unit_price_inc_tax = $uf_data ? $this->num_uf($product['unit_price_inc_tax']) : $product['unit_price_inc_tax'];
                    ...
                    'unit_price' => $uf_unit_price,
                    'line_discount_type' =>  null,
                    'line_discount_amount' =>  $discount_amount ??0,
        ...
        foreach ($payments as $payment) {
            ...
            if (!empty($payment['payment_id'])) {
                ...
                if ($payment_amount != 0) {
                    ...
                    }elseif ($payment['method'] == 'bkash') {
                        $payment_data['transaction_no'] = $payment['transaction_no_5'];
                    }elseif ($payment['method'] == 'rocket') {
                        $payment_data['transaction_no'] = $payment['transaction_no_6'];
                    }elseif ($payment['method'] == 'nogod') {
                        $payment_data['transaction_no'] = $payment['transaction_no_7'];
                    }

                    elseif ($payment['method'] == 'advance') {
                        $payment_data['transaction_no'] = $payment['transaction_no_8'];
                    }
        ...
        if (!empty($payments_formatted)) {
            ...
            foreach ($transaction->payment_lines as $key => $value) {
                if (!empty($account_transactions[$key])) {
                    ...
                    $this->accountLink($value->id,$value->account_id);
                    }
    ...
    public function getExpenseTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
        ...
        $output['total_expense'] = $sell_details->sum('final_total');
        $output['expense_due'] = $sell_details->sum('final_total') - $sell_details->sum('total_paid');
        return $output;
    }

    public function getIncomeTotals($business_id, $start_date = null, $end_date = null, $location_id = null, $created_by = null)
    {
        $query = Transaction::where('transactions.business_id', $business_id)
        ...
        $output['total_income'] = $sell_details->sum('final_total');
        $output['income_due'] = $sell_details->sum('final_total') - $sell_details->sum('total_paid');
        return $output;
    }
    ...
    */
     ...
     * @return Obj
     */
    public function getExpenseReport($business_id,$filters = [],$type = 'by_category', $transaction_type=null) {
        ...
        foreach ($transaction_lines as $line) {
            ...
            if ($stop_selling_expired && empty($purchase_line_id)) {
                ...
                $query->where(function($q) use($expiry_date){
                    $q->whereNull('PL.exp_date')->orwhere('PL.exp_date', '>', $expiry_date);
                });
            }
    ...
    public function getOpeningClosingStock($business_id, $date, $location_id, $is_opening = false)
    {
        ...
        $query->select(
            DB::raw("SUM(
                            ...
                            date(sale.transaction_date) <= '$date') ) * (v.dpp_inc_tax)
                        ) as stock")
        );
    ...
    public function getClosingStockByPurchasePrice($business_id, $date, $location_id, $is_opening = false)
    {
        $query = PurchaseLine::join(
            'transactions as purchase',
            'purchase_lines.transaction_id',
    ...
    public function getClosingStockBySellPrice($business_id, $date, $location_id, $is_opening = false)
    {
        $query = PurchaseLine::join(
            'transactions as purchase',
            'purchase_lines.transaction_id',
        ...
        $query->select(
            DB::raw("SUM(
                            ...
                            tsl.transaction_id=sale.id
                            WHERE tspl.purchase_line_id = purchase_lines.id AND
                            date(sale.transaction_date) <= '$date') ) * (v.sell_price_inc_tax)
                        ) as stock")
        );
    ...
    */
    ...
    public function getTransactionTotals(
        ...
        if (in_array('income', $transaction_types)) {
            $query->addSelect(
                DB::raw("SUM(IF(transactions.type='income', final_total, 0)) as total_income")
            );
        }
        ...
        if (in_array('income', $transaction_types)) {
            $output['total_income'] =
                !empty($transaction_totals->total_income) ?
                $transaction_totals->total_income : 0;
        }
